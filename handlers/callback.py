...
def updateCallback(client, callback_query,redis):
  ...
  if group is True and int(date[2]) == userID and not redis.get("{}Nbot:floodUsers:{}".format(BOT_ID,userID)):
    ...
    if date[0] == "dlf":
      File = date[1]
      os.system("rm ./files/"+File)
      url = "https://raw.githubusercontent.com/whatisnormalx/ald-files/master/"+File
    ...
    if date[0] == "ShowDateUser":
      ...
      age = getAge(userID,r)
      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(Name(userFN),url="t.me/ald_source")],[InlineKeyboardButton(r.Rrank.format(t),url="t.me/ald_source")],[InlineKeyboardButton(r.Rmsgs.format(msgs),url="t.me/ald_source")],[InlineKeyboardButton(r.Rrate.format(str(rate)+"%"),url="t.me/ald_source")],[InlineKeyboardButton(r.Redits.format(edits),url="t.me/ald_source")],[InlineKeyboardButton(r.Rage.format(age),url="t.me/ald_source")]])
